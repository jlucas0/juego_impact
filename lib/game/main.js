...
.defines(function(){
	...
	MyGame = ig.Game.extend({
		...
		gravity: 150,
		levels: [[LevelLevel2,40,'musicA'],[LevelLevel1,50,'musicB'],[LevelLevel3,100,'musicA']], //Niveles, tiempo en segundos para cada uno y música de fondo
		timer: undefined,
		lastTime: undefined,
		currentLevel: -1,
		titleBlink: false,
		...
		draw: function() {
			...
			if(this.currentLevel>=0){
				...
				this.font.draw( 'Energia: '+this.player.points, 50, 10, ig.Font.ALIGN.CENTER );
				...
				if(this.paused){
					...
					if(this.end){
						this.font.draw( "ENERGIA FINAL", ig.system.width/2, ig.system.height / 2  - 20, ig.Font.ALIGN.CENTER );
		...
		levelLoader: function(){
            ...
            this.player.addPoints(this.startEnergy);
		...
		endLevel: function(){
			this.startEnergy = this.player.points;
			this.pauseGame();
			this.end = true;
		}
	});
